...
public class Macro_Builder_Batch_CountTest {
    ...
    public void pluginCommandWritesBatchCountCsv() throws Exception {
        ...
        File filterMacro = new File(exportFolder, "Batch_Count_Filter.ijm");
        File settingsJson = new File(exportFolder, "Batch_Count.settings.json");
        Files.write(filterMacro.toPath(), "// no-op\n".getBytes(StandardCharsets.UTF_8));
        Files.write(settingsJson.toPath(), BatchMacroExporter.toJson(
                new BatchMacroExporter.ExportedSettings(
                        settingsJson,
                        filterMacro.getName(),
                        BatchMacroExporter.DEFAULT_RESULTS_FILE,
                        fixedSettings(100.0))).getBytes(StandardCharsets.UTF_8));
        ...
        new Macro_Builder_Batch_Count().run(
                "settings=[" + settingsJson.getAbsolutePath() + "] "
