...
public final class ImagePreviewPanel extends JPanel {
    ...
    private PreviewDisplaySettings displaySettings =
    ...
    private ImageProcessor currentProcessor() {
        ...
        if (!colorProcessor) {
            ColorModel colorModel = displaySettings.getLutMode() == PreviewDisplaySettings.LutMode.GREY
                    ? greyColorModel()
                    : userColorModel(imp, copy);
            if (colorModel != null) {
    ...
    private ColorModel userColorModel(ImagePlus imp, ImageProcessor fallbackProcessor) {
        try {
            LUT[] luts = imp.getLuts();
            int index = clamp(currentC, 1, Math.max(1, imp.getNChannels())) - 1;
            if (luts != null && index >= 0 && index < luts.length && luts[index] != null) {
                return luts[index];
            }
        } catch (RuntimeException ignored) {
        }
        try {
            return fallbackProcessor == null ? null : fallbackProcessor.getColorModel();
        } catch (RuntimeException ignored) {
            return null;
        }
    }

    private static ColorModel greyColorModel() {
