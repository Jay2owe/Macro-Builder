...
public final class DagCanvasPanel extends JPanel {
    ...
    public DagCanvasPanel(SandboxModel model, CatalogSupplier catalogSupplier,
        ...
        this(model, catalogSupplier, nodeCreator, null, null, null, selectionCallback, changeCallback);
    }
    ...
    public DagCanvasPanel(SandboxModel model, CatalogSupplier catalogSupplier,
                          ...
                          Runnable selectionCallback, Runnable changeCallback) {
        this(model, catalogSupplier, nodeCreator, nodeActionHandler, null, null, selectionCallback, changeCallback);
    }
    ...
    public DagCanvasPanel(SandboxModel model, CatalogSupplier catalogSupplier,
                          ...
                          Runnable selectionCallback, Runnable changeCallback) {
        this(model, catalogSupplier, nodeCreator, nodeActionHandler, combinerActionHandler, null,
                selectionCallback, changeCallback);
    }
    ...
    void refreshUndoControl() {
        if (undoButton == null || undoActionHandler == null) return;
        undoButton.setEnabled(undoActionHandler.canUndo());
    }
    ...
    private JPanel buildUndoRow() {
        JPanel row = new JPanel(new BorderLayout(8, 0));
        row.setOpaque(true);
        row.setBackground(new Color(255, 248, 221));
        row.setBorder(BorderFactory.createCompoundBorder(
                BorderFactory.createLineBorder(new Color(203, 145, 45), 2),
                BorderFactory.createEmptyBorder(6, 8, 6, 8)));

        JLabel label = new JLabel("Undo");
        label.setFont(label.getFont().deriveFont(Font.BOLD, 12f));
        label.setForeground(new Color(115, 76, 20));
        row.add(label, BorderLayout.WEST);

        undoButton = new JButton("Undo last builder change");
        undoButton.setOpaque(true);
        undoButton.setFocusPainted(false);
        undoButton.setFont(undoButton.getFont().deriveFont(Font.BOLD, 12f));
        undoButton.setForeground(new Color(92, 59, 11));
        undoButton.setBackground(Color.WHITE);
        undoButton.setBorder(BorderFactory.createCompoundBorder(
                BorderFactory.createLineBorder(new Color(150, 100, 25), 2),
                BorderFactory.createEmptyBorder(4, 10, 4, 10)));
        undoButton.setToolTipText("Undo the last builder change (Ctrl+Z)");
        undoButton.setPreferredSize(new Dimension(210, 30));
        undoButton.addActionListener(e -> {
            if (undoActionHandler != null) undoActionHandler.undoLastChange();
        });
        row.add(undoButton, BorderLayout.CENTER);
        refreshUndoControl();
        return row;
    }
