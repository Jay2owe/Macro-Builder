...
`Preview to selected point` runs only up to the selected step or merge card. `Preview full filter` runs the whole builder chain. Both preview buttons update the embedded output preview and leave the selected source image unchanged. `Large view` opens the source and output previews in a larger side-by-side window. `Brightness/Contrast` opens live display controls for the source and preview panes, and `Grey LUT` switches both panes between grey and the LUT selected in Fiji. These display controls do not change the macro or image data. The source and output preview Z sliders stay synced in both the embedded view and the large view, so moving to a slice in either preview shows the matching slice in the other preview.
...
Click `Save as batch macro...` to save a self-contained batch count macro for the current macro and latest count settings. Macro Builder asks whether to save in Fiji's macros folder or another location. The saved `.ijm` file contains the filter steps, primary channel, count mode, threshold mode, fixed thresholds, size filters, and output CSV name. When you run it, it asks for an input folder and output folder, then runs `Macro Builder Batch Count`.
